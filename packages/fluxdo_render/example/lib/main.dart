          ...
          actions: [
            IconButton(
              ...
              onPressed: () => Navigator.of(
                context,
              ).push(MaterialPageRoute(builder: (_) => const EditorDemoPage())),
            ),
            IconButton(
  ...
  switch (node) {
    ...
    case ListNode():
      ...
      for (int i = 0; i < node.items.length; i++) {
        ...
        buf.writeln(
          '$pad  [$i] ${item.inlines.length} inlines'
...
Widget _galleryEmojiBuilder(BuildContext context, EmojiRun emoji, double size) {
  ...
  return SizedBox(
    ...
    height: size,
    child: Center(
      child: Text(unicode, style: TextStyle(fontSize: size, height: 1.0)),
    ),
  );
}
