...
void main() {
  ...
  testWidgets('reposition 选区滚动 → toolbar 跟随移动,且始终夹在视口内可见', (tester) async {
    final t = await mountToolbar(tester, onQuote: (_) {});
    t.show(dataAt(const Rect.fromLTWH(100, 300, 80, 20)));
    await tester.pump();
    ...
    expect(y2, greaterThanOrEqualTo(0.0), reason: 'toolbar 夹在视口内,始终可见(不滑出屏幕)');
    t.hide();
  });

  testWidgets('选区比视口还高(顶/底都出界,中段可见)→ toolbar 仍在视口内可见', (tester) async {
    final t = await mountToolbar(tester, onQuote: (_) {});
    final viewH =
    ...
    expect(pos.dy, lessThan(viewH), reason: '选区跨满视口时,工具栏夹到视口内,始终可见');
    t.hide();
  });

  testWidgets('选区完全滚出视口(无可见块)→ toolbar 隐藏(滚回再现)', (tester) async {
    final t = await mountToolbar(tester, onQuote: (_) {});
    t.show(dataAt(const Rect.fromLTWH(100, 300, 80, 20)));
    await tester.pump();
    expect(copyText, findsOneWidget);
  ...
  testWidgets('reposition yCompensation 抵消滚动滞后(toolbar 预平移 delta)', (
    ...
    t.reposition(
      dataAt(const Rect.fromLTWH(100, 400, 80, 20)),
      yCompensation: 60,
    );
    await tester.pump();
    final y1 = tester.getTopLeft(copyText).dy;
  ...
  testWidgets('上方放不下 → toolbar 翻到选区下方(SDK delegate)', (tester) async {
    ...
    await tester.pump();
    final pos = tester.getTopLeft(copyText);
    expect(pos.dy, greaterThan(30), reason: '上方放不下应翻到选区下方(bottom=30 之下)');
    t.hide();
  });
  ...
  testWidgets('选区靠屏幕右边 → toolbar 夹回视口内(SDK delegate)', (tester) async {
    final t = await mountToolbar(tester, onQuote: (_) {});
    final screenW =
