...
void main() {
  ...
  group('逻辑块表(回收块)', () {
    test('全部块无 live 句柄(全回收)→ expandSelection + 复制仍完整', () {
      ...
      final sel = DocumentSelection(
        base: const DocumentPosition(
          blockId: SelectableBlockId(0),
          renderOffset: 1,
        ),
        extent: const DocumentPosition(
          blockId: SelectableBlockId(2),
          renderOffset: 2,
        ),
      );
    ...
    test('跨 chunk 文档序(chunkIndex, docOrder)排序正确', () {
      ...
      expect(
        [for (final b in order) b.projection.projectAll()],
        ['A', 'B', 'C', 'D'],
      );
      ...
      final sel = DocumentSelection(
        ...
        extent: const DocumentPosition(
          blockId: SelectableBlockId(1, chunkIndex: 1),
          renderOffset: 1,
        ),
      );
      final data = SelectionExporter(reg).export(sel);
  ...
  testWidgets('中间段落被回收(widget 移除)后,跨段复制仍含中间段', (tester) async {
    ...
    c.selection = sel;
    expect(
      SelectionExporter(c.registry).export(sel)!.plainText,
      'AAA\nBBB\nCCC',
    );
