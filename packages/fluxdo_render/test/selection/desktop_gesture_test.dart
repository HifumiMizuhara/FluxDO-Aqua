...
void main() {
  ...
  testWidgets('renderLengthOf 返回块渲染长度', (tester) async {
    ...
    expect(
      SelectionHitTester(
        c.registry,
      ).renderLengthOf(const SelectableBlockId(999)),
      isNull,
    );
  });
  ...
  testWidgets('多击残留计数后拖拽:从拖拽起点重锚,非整段', (tester) async {
    final c = SelectionController(SelectionRegistry());
    await tester.pumpWidget(
      host(c, const [TextRun('AAAAA BBBBB CCCCC DDDDD EEEEE FFFFF GGGGG')]),
    );
    await tester.pumpAndSettle();
    final para = textGeometryAt(tester).renderBox;
    ...
    expect(lo, greaterThan(0), reason: '拖拽起点重锚,base 不应残留为段首(整段)');
  });
}
