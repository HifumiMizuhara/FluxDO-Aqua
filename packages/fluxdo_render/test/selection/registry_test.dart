...
void main() {
  ...
  testWidgets('N 个段落 → registry 注册 N 个块', (tester) async {
    ...
    await tester.pumpWidget(
      wrap(controller, [para('第一段', 0), para('第二段', 1), para('第三段', 2)]),
    );
  ...
  testWidgets('orderedBlocks 按文档序(docOrder)升序', (tester) async {
    final controller = SelectionController(SelectionRegistry());
    await tester.pumpWidget(
      wrap(controller, [para('上', 0), para('中', 1), para('下', 2)]),
    );
    await tester.pumpAndSettle();
    ...
    for (var i = 1; i < order.length; i++) {
      expect(
        order[i].id.docOrder > order[i - 1].id.docOrder,
        isTrue,
  ...
  testWidgets('无 SelectionScope 时不崩(退化不可选)', (tester) async {
    await tester.pumpWidget(MaterialApp(home: Scaffold(body: para('无选区上下文'))));
    await tester.pumpAndSettle();
