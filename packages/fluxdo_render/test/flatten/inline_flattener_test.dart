...
void main() {
  ...
  group('InlineCodeRun', () {
    ...
    testWidgets(
      'light 主题:派生 onSurfaceVariant 字色(背景移到 painter,span 不带 background)',
      (tester) async {
        late BuildContext ctx;
        await tester.pumpWidget(
          MaterialApp(
        ...
        expect(span.style?.fontFamilyFallback, [
          'monospace',
          'Menlo',
          'Courier',
        ]);
    ...
    test('无 context 时 color 退化为 null(背景始终在 painter,span 无 background)', () {
      final result = flattener.flatten([const InlineCodeRun('x')], baseStyle);
      final span = codeLeafOf(result);
  ...
  group('EmojiRun', () {
    testWidgets('普通 emoji 产出 WidgetSpan,size = baseStyle.fontSize', (
      tester,
    ) async {
      late BuildContext ctx;
      double? capturedSize;
