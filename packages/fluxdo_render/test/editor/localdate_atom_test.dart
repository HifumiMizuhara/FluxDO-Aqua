...
void main() {
  ...
  test('date 原子周围编辑:退格整删原子', () {
    ...
    state.updateSelection(
      EditorSelection.collapsed(EditorPosition(blockId: tb.id, offset: 2)),
    );
    state.backspace();
  ...
  testWidgets('编辑态渲染 date chip 不崩(WidgetSpan 原子)', (tester) async {
    ...
    final tb = state.blocks.single as TextBlock;
    state.updateSelection(
      EditorSelection.collapsed(EditorPosition(blockId: tb.id, offset: 0)),
    );
    for (var i = 0; i < 6; i++) {
  ...
  test('replaceAtomAt:换 date 原子属性,undo 一步', () {
    ...
    state.replaceAtomAt(
      tb.id,
      0,
      const LocalDateRun(date: 'x', fallbackText: 'x'),
    );
    expect(
      ((state.blocks.single as TextBlock).content.atoms[1] as LocalDateRun)
          .date,
      '2027-05-05',
    );
    state.undo();
