...
EditableTextContent content(String text, {List<MarkSpan> marks = const []}) =>
...
void main() {
  ...
  group('caret 重映射(guardAtCaret: false,收口全折场景)', () {
    ...
    test('守卫开(默认):caret 在字面对内部不折(光标驻留展开态)', () {
      ...
      expect(identical(r.content, src), isTrue, reason: '光标在字面内 = 正在编辑,保持字面');
    });
  ...
  group('排除与约束', () {
    test('inlineCode mark 覆盖区内的字面不折叠', () {
      final src = content(
        'code **x** here',
        marks: const [MarkSpan(start: 0, end: 15, kind: MarkKind.inlineCode)],
      );
    ...
    test('BBCode 内容含 [ 不折;HTML 内容含 < 不折', () {
      ...
      expect(
        spinInlineMarks(content('<small>a<b</small>'), caret: 0).content.marks,
        isEmpty,
      );
    });
    ...
    test('link 字面折叠为 link mark(caret 守卫:光标在内不折)', () {
      ...
      final folded = spinInlineMarks(
        content('a [text](https://x) b'),
        caret: 0,
      );
      expect(folded.content.text, 'a text b');
      final link = folded.content.marks.firstWhere(
        (m) => m.kind == MarkKind.link,
      );
      expect(link.attr, 'https://x');
      // 光标在字面区间内部(正在编辑 href)→ 守卫挡住不折
      final guarded = spinInlineMarks(
        content('a [text](https://x) b'),
        caret: 12,
      );
  ...
  group('v2:mark 与字面混合重解析(物化 → 重折叠)', () {
    ...
    test('strong 内容中补 ~~ 对 → 嵌套折叠(光标在外/收口)', () {
      ...
      expect(held.content.text, '**axb**', reason: '内层 ~~x~~ 折叠,外层(光标驻留)保持字面');
    ...
    test('inlineCode 不物化:code 内容里的 * 不与外部字面配对', () {
      ...
      final r = spinInlineMarks(
        content(
          'a*b *x* d',
          marks: const [MarkSpan(start: 0, end: 3, kind: MarkKind.inlineCode)],
        ),
        caret: 9,
      );
      ...
      final code = r.content.marks.firstWhere(
        (m) => m.kind == MarkKind.inlineCode,
      );
      expect(code.start, 0);
