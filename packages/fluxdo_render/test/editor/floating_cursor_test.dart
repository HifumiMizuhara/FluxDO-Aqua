...
void main() {
  testWidgets('Start 出幽灵 → Update 吸附移光标 → End 收幽灵留光标', (tester) async {
    ...
    expect(caretDim.color, theme.colorScheme.outline, reason: '浮动期间实光标灰化残影');
    expect(caretDim.alwaysVisible, isTrue, reason: '残影常亮不闪');
    final ghostBefore = tester.getTopLeft(find.byKey(kFloatingCursorGhostKey));

    await sendFloating(tester, 'update', offset: const Offset(150, 0));
    final ghostAfter = tester.getTopLeft(find.byKey(kFloatingCursorGhostKey));
    expect(ghostAfter.dx, greaterThan(ghostBefore.dx), reason: '幽灵跟手右移');
    ...
    expect(caretBack.color, theme.colorScheme.primary, reason: 'End 恢复主题色');
  });
  ...
  testWidgets('虚拟指针:start/moveBy 二维漂移吸附,end 收幽灵', (tester) async {
    ...
    final state = EditorState.fromTexts([
      ...
      'second line target words',
    ]);
    addTearDown(state.dispose);
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: FluxdoEditor(state: state, autofocus: true, virtualPointer: vp),
        ),
      ),
    );
    await tester.pump();
