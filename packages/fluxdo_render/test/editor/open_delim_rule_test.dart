...
void main() {
  ...
  group('补打开定界符触发', () {
    for (final (delim, kind) in cases) {
      test('$delim → $kind', () {
        ...
        final out = tryApplyInputRules(
          s,
          'b0',
          typedChar: delim[delim.length - 1],
        );
        ...
        expect(s.selection!.extent.offset, 0, reason: '光标本来就在内容首,不该被甩到尾巴');
      });
    }
  });
    ...
    test('右边没有闭定界符', () {
      final s = EditorState(
        blocks: [
          TextBlock(
            id: 'b0',
            content: EditableTextContent(text: '~~诚邀你测试'),
          ),
        ],
      );
      addTearDown(s.dispose);
      ...
      expect(
        tryApplyInputRules(s, 'b0', typedChar: '~'),
    ...
    test('内容为空(`~~~~`)', () {
      final s = EditorState(
        blocks: [
          TextBlock(
            id: 'b0',
            content: EditableTextContent(text: '~~~~'),
          ),
        ],
      );
      addTearDown(s.dispose);
    ...
    test('闭定界符在下一行(不跨软换行)', () {
      final s = EditorState(
        blocks: [
          TextBlock(
            id: 'b0',
            content: EditableTextContent(text: '~~诚邀\n你测试~~'),
          ),
        ],
      );
      addTearDown(s.dispose);
    ...
    test('内容首是空格(CommonMark 语义)', () {
      final s = EditorState(
        blocks: [
          TextBlock(
            id: 'b0',
            content: EditableTextContent(text: '~~ 测试~~'),
          ),
        ],
      );
      addTearDown(s.dispose);
  ...
  test('原有路径不受影响:打完闭定界符仍然触发,光标在内容尾', () {
    ...
    addTearDown(s.dispose);
    s.updateSelection(
      const EditorSelection.collapsed(EditorPosition(blockId: 'b0', offset: 6)),
    );
