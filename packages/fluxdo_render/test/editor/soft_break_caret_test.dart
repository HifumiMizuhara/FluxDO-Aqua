...
void main() {
  Future<(EditorState, List<Rect?>)> boot(WidgetTester tester) async {
    ...
    final state = EditorState(
      blocks: [
        TextBlock(
          id: 'e_0',
          content: EditableTextContent(text: '细说'),
        ),
      ],
    );
  ...
  testWidgets('软换行后光标不该乱跳:与随后打字的位置一致', (tester) async {
    ...
    expect(
      ...
      reason:
          '空行光标 top=${onEmptyLine.top},打字后 top=${afterTyping.top}'
          '(换行前 top=${beforeBreak.top})',
    );
    expect(onEmptyLine.top, greaterThan(beforeBreak.top), reason: '换行后应该在下一行');
