...
String docText(EditorState s) =>
    s.blocks.whereType<TextBlock>().map((b) => b.content.text).join('\n');

void main() {
  testWidgets('富产物优先:importer 返回块 → 直接插入,不读纯文本', (tester) async {
    final state = await pumpEditor(
      tester,
      richPasteImporter: () async => [
        TextBlock(
          id: 'r_0',
          content: EditableTextContent(text: 'RICH'),
        ),
      ],
    );
  ...
  testWidgets('importer 返回 null → 回落纯文本粘贴', (tester) async {
    final state = await pumpEditor(tester, richPasteImporter: () async => null);
    await pressPaste(tester);
