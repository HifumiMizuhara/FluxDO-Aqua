...
void main() {
    ...
    test('粗体/斜体/删除线/行内代码/下划线', () {
      expect(
        docToMarkdown([
          tb(
            'abc',
            marks: const [MarkSpan(start: 0, end: 3, kind: MarkKind.strong)],
          ),
        ]),
        '**abc**',
      );
      ...
      expect(
        docToMarkdown([
          tb(
            'u',
            marks: const [MarkSpan(start: 0, end: 1, kind: MarkKind.underline)],
          ),
        ]),
        '[u]u[/u]',
      );
    ...
    test('cooked 摊平 → 序列化:嵌套方向端到端保序', () {
      ...
      final reversed = EditableTextContent.fromInlines(const [
        StyledRun(
          kind: InlineStyleKind.lineThrough,
          children: [
            StyledRun(kind: InlineStyleKind.small, children: [TextRun('旧称')]),
          ],
        ),
      ]);
      expect(
  ...
  group('原子', () {
    test('emoji/mention', () {
      ...
      expect(
        docToMarkdown([
          tb('hi $kAtomChar $kAtomChar', atoms: const {3: emoji, 5: mention}),
        ]),
        'hi :smile: @sam',
      );
    });
  ...
  group('块类型', () {
    ...
    test('块间空行连接', () {
      expect(docToMarkdown([tb('一'), tb('二')]), '一\n\n二');
    });
  ...
  group('孤岛', () {
    test('代码块(含 fence 升级)', () {
      ...
      expect(
        serializeIslandNode(const CodeBlockNode(id: 'b', code: 'a```b')),
        '````\na```b\n````',
      );
    });
  ...
  group('岛覆盖面(M4 编辑已有帖子)', () {
    ...
    test('video/audio:upload 短链优先,直链裸 URL', () {
      ...
      expect(
        serializeIslandNode(const AudioNode(id: 'b', src: 'upload://y.mp3')),
        '![|audio](upload://y.mp3)',
      );
    });
    ...
    test('行内:attachment/hashtag/inline-onebox/date/行内公式/spoiler', () {
      ...
      expect(
        serializeIslandNode(
          const ParagraphNode(id: 'b', inlines: [MathInlineRun('x^2')]),
        ),
        '\$x^2\$',
      );
      expect(
      ...
      expect(
        serializeIslandNode(
          const ParagraphNode(
            id: 'b',
            inlines: [FootnoteRefRun(number: '1', fnId: 'fn1')],
          ),
        ),
        '[^1]',
      );
      ...
      expect(
        serializeIslandNode(
          const ParagraphNode(
            ...
            inlines: [
              StyledRun(
                kind: InlineStyleKind.superscript,
                children: [TextRun('2')],
              ),
...
void _bareUrlTests() {
  ...
  String md(String text, MarkSpan m) => docToMarkdown([
    TextBlock(
      id: 'b0',
      content: EditableTextContent(text: text, marks: [m]),
    ),
  ]);
  ...
  group('裸 URL 序列化', () {
    test('锚文本与 href 只差 http:// → 写回裸 URL(linkify 产物)', () {
      expect(
        md('看 dl.google.com 吧', link(2, 15, 'http://dl.google.com')),
        '看 dl.google.com 吧',
      );
    });
    ...
    test('锚文本与 href 差 https:// → 保留完整语法(只能是手写链接)', () {
      expect(
        md('看 dl.google.com 吧', link(2, 15, 'https://dl.google.com')),
    ...
    test('锚文本与 href 只差 mailto: → 写回裸邮箱(linkify 产物)', () {
      expect(
        md('联系 user@example.com 吧', link(3, 19, 'mailto:user@example.com')),
    ...
    test('真·带文字的链接不受影响', () {
      expect(md('看 这里 吧', link(2, 4, 'https://a.b')), '看 [这里](https://a.b) 吧');
      // 锚文本是另一个地址(不是同一 URL 的裸形态)→ 不能当裸 URL
      expect(
        md('看 a.com 吧', link(2, 7, 'https://b.com')),
