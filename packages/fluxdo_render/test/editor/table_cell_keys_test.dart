...
void main() {
  ...
  testWidgets('双光标回归:cell 编辑时编辑器 caret 消失、点 cell 不动编辑器选区', (tester) async {
    var n = 0;
    final state = EditorState(
      blocks: blockNodesToDoc(
        ParagraphParser().parse(
  ...
  testWidgets('选择柄整选表格:描边 + 退格删整表', (tester) async {
    ...
    await tester.pump();
    expect(
      state.blocks.whereType<IslandBlock>(),
      isEmpty,
      reason: '整选态退格应删整个表格',
    );
  ...
  testWidgets('触屏(无鼠标)cell 编辑态:行/列柄与加条常显', (tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: SingleChildScrollView(
            child: FluxdoEditor(state: state, onTableEdited: (island, md) {}),
          ),
        ),
      ),
    );
