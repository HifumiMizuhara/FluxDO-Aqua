...
EditorState fromHtml(String html) {
  var n = 0;
  return EditorState(
    blocks: blockNodesToDoc(ParagraphParser().parse(html), () => 'e_${n++}'),
  );
}
...
void main() {
  testWidgets('弹层位移后岛 Element 复用(不 deactivate 重建)', (tester) async {
    ...
    final islandStateBefore = tester.state(find.byType(EditorIsland));
    ...
    expect(
      ...
      isTrue,
      reason:
          '岛未被删除,Element/State 必须复用 —— 重建说明 children '
