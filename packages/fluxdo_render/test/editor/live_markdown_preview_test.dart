...
void main() {
  ...
  group('ir 进入物化/离开折叠(状态层)', () {
    ...
    test('光标落进 mark → 整簇物化为字面,caret 映射到字面坐标', () {
      ...
      expect(s.selection!.extent.offset, 7, reason: '开定界符 `**` 在光标前插入 → +2');
    ...
    test('延迟收口:down 落进 mark 不物化,定性单击后才物化', () {
      ...
      expect(first(s).content.text, 'ab bold cd', reason: 'down 阶段形态不变');
      ...
      expect(first(s).content.text, 'ab bold cd', reason: '选择存续期间不展开');
      expect(first(s).content.marks, hasLength(1));
    ...
    test('边界含语义:贴 start/end 也进入物化,光标落定界符外侧', () {
      ...
      expect(sStart.selection!.extent.offset, 3, reason: 'start 处物化停开定界符前(外侧)');

      final sEnd = makeState(midMark());
      caretAt(sEnd, 7); // 贴 end
      expect(first(sEnd).content.text, 'ab **bold** cd');
      expect(
        sEnd.selection!.extent.offset,
        11,
    ...
    test('字面区内左右移动逐字符;移出区间边界即折叠', () {
      ...
      expect(first(s).content.text, 'ab **bold** cd', reason: '仍在区间内部,守卫保持展开');
      s.moveCaretHorizontal(1); // 11 = 区间边界:闭区间守卫,仍驻留
      expect(
        first(s).content.text,
        'ab **bold** cd',
    ...
    test('undo:字面区内编辑记历史,快照回到编辑时刻的物化态', () {
      ...
      s.undo();
      final c = first(s).content;
      expect(c.text, '**bold**', reason: 'undo 快照 = 编辑时刻的物化态字面(折叠不进历史)');
      expect(c.marks, isEmpty);
      expect(s.selection!.extent.offset, 4, reason: '光标回编辑点 = 守卫成立,字面保持');
  ...
  group('反向固化:显形零模型泄漏', () {
    ...
    test('纯 emoji 段显形不掉出大表情档', () {
      ...
      final content = EditableTextContent(
        text: '￼',
        marks: const [MarkSpan(start: 0, end: 1, kind: MarkKind.spoilerInline)],
        atoms: {0: emoji},
      );
      final inlines = content.toInlines(forEditing: true, revealMarkdownAt: 0);
  ...
  group('widget 级:焦点/选区/开关/IME', () {
    ...
    testWidgets('可物化 mark:光标进出时字面出现/消失(物化非投影)', (tester) async {
    ...
    testWidgets('inlineCode:投影显形出现/消失,range 立即折叠', (tester) async {
    ...
    testWidgets('ir:Shift 扩选期间物化字面保持(真实文本,布局稳定)', (tester) async {
    ...
    testWidgets('ir:mark 态起手的程序化 range 不物化(选择中无格式符)', (tester) async {
      // 光标先放 mark 外(不物化)
      final state = await pumpEditor(
        tester,
        content: strongContent(),
      ...
      await tester.pump();
      expect(paragraphText(tester), 'bold tail', reason: '选择态不物化不显形');
    ...
    testWidgets('composing 期间不显形(IME 预编辑不受扰,inlineCode 投影)', (tester) async {
      final state = await pumpEditor(
        tester,
        content: codeContent(),
        caretOffset: 4,
    ...
    testWidgets('IME 窗口喂的文本 = content.text 原文(投影定界符不混入)', (tester) async {
      final state = await pumpEditor(
        tester,
        content: codeContent(),
        caretOffset: 4,
