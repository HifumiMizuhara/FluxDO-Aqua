...
const _bare = ImageRun(
  src: 'https://idcflare.com/uploads/x.png',
  alt: 'doge.png',
  width: 28,
  height: 28,
);
...
void main() {
  test('白名单:全部图片可编辑(官方 inline:true,无图片岛)', () {
    ...
    final doc = blockNodesToDoc([
      const ParagraphNode(
        id: 'b_0',
        inlines: [TextRun('看这个 '), _bare, TextRun(' 表情')],
      ),
    ...
    expect(docToMarkdown([doc[1]]), '![|690x388, 50%](upload://a.jpeg)');
    expect(docToMarkdown([doc[2]]), '![|690x388](upload://b.jpeg)');
  });
  ...
  test('序列化写回标准图片语法(尺寸保留)', () {
    final doc = blockNodesToDoc([
      const ParagraphNode(
        id: 'b_0',
        inlines: [TextRun('看 '), _bare, TextRun(' 后文')],
      ),
    ], _idGen());
    expect(
      docToMarkdown(doc),
  ...
  test('图后打字:光标在原子后插入文本,原子身份不动', () {
    final s = EditorState(
      blocks: [
        TextBlock(
          id: 'e_0',
          content: EditableTextContent.fromInlines(const [TextRun('a'), _bare]),
        ),
      ],
    );
  ...
  testWidgets('段高被图撑起(strut 强制是双向钳制,含图段必须放开)', (tester) async {
    // 153x153 大图(用户实测形态:heart webp 表情)
    const big = ImageRun(
      src: 'https://cdn3.ldstatic.com/x.webp',
      alt: '❤️_20.webp',
      width: 153,
      height: 153,
    );
