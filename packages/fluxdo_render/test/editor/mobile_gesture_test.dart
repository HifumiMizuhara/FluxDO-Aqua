...
void main() {
  testWidgets('触摸竖滑 = 页面滚动,选区不变(pan 不进竞技场)', (tester) async {
  ...
  testWidgets('长按 = 选词 + 手柄 + 动作条;触摸后收 UI', (tester) async {
    ...
    final g = await tester.startGesture(target, kind: PointerDeviceKind.touch);
  ...
  testWidgets('长按图片原子 = 整选(不选词不弹词边界)', (tester) async {
    ...
    final (_, state) = await pumpMobile(
      tester,
      blocks: [
        TextBlock(
          id: 'e_0',
  ...
  testWidgets('长按岛(代码块)= 让路不误选邻段', (tester) async {
    final (_, state) = await pumpMobile(
      ...
      blocks: [
        TextBlock(
          id: 'e_0',
          content: EditableTextContent(text: '邻段文字'),
        ),
        const IslandBlock(
          id: 'e_isl',
  ...
  testWidgets('普通触摸落光标 = 不出动作条(仅长按空白才出)', (tester) async {
    final (_, state) = await pumpMobile(tester);
    final para = tester.getRect(find.textContaining('hello').first);
  ...
  testWidgets('触摸落光标 = collapsed 手柄出现;鼠标点击不出', (tester) async {
    ...
    expect(state.selection!.isCollapsed, isTrue);
    expect(
      find.byKey(kCollapsedHandleKey),
      findsOneWidget,
      reason: '触摸落光标出单手柄',
    );
    ...
    await tester.pump();
    await tester.pump();
    expect(find.byKey(kCollapsedHandleKey), findsNothing, reason: '鼠标来源不出手柄');
  });
  ...
  testWidgets('拖 collapsed 手柄到视口底缘 = 边缘自动滚', (tester) async {
    ...
    expect(scroll.offset, greaterThan(offsetAtEdge), reason: '贴边持续自动滚动');
    await g.up();
    await tester.pump();
