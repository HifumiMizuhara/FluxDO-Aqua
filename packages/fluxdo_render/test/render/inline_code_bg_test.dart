...
void main() {
  ...
  testWidgets('行内 <code> 段落挂 InlineCodeBackgroundPainter(圆角灰底)', (
    ...
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(
          body: FluxdoRender(cookedHtml: '<p>前 <code>inline code</code> 后</p>'),
        ),
      ),
    );
    await tester.pumpAndSettle();
    expect(codeBgPainter(), findsWidgets, reason: '含行内 code 的段落应挂背景 painter');
  });

  testWidgets('代码块 <pre><code> 不挂行内代码背景 painter(整块自带背景)', (tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(
          body: FluxdoRender(
