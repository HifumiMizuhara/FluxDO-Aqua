...
void main() {
  ...
  testWidgets('包裹 li(空 inline + 嵌套 ol/ul):marker 与嵌套首行同排(merge)', (
          ...
          ListItem(
            ...
            children: [
              ...
              ListNode(
                ...
                depth: 1,
                items: [
                  ListItem(inlines: [TextRun('BASE_URL')]),
                ],
              ),
            ],
          ),
    ...
    expect(gap, lessThan(18.0), reason: '○ 与 1. 间距应紧凑(对齐网页),实际 gap=$gap');
  });

  testWidgets('无序 marker 按 depth 变形:disc/circle/square(绘制形状)', (tester) async {
    await pump(
      tester,
      const ListNode(
        id: 'l',
    ...
    expect(
      find.byKey(const ValueKey('ul_marker_disc')),
      findsOneWidget,
      reason: 'depth0 = disc',
    );
    expect(
      find.byKey(const ValueKey('ul_marker_circle')),
      findsOneWidget,
      reason: 'depth1 = circle',
    );
    expect(
      find.byKey(const ValueKey('ul_marker_square')),
      findsOneWidget,
        ...
        home: Scaffold(
          body: Builder(
            builder: (ctx) {
              final f = NodeFactory();
              return ListView(
                children: [for (final n in nodes) f.build(ctx, n)],
              );
            },
          ),
        ),
