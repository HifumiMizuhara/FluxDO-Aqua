...
class TextBlock extends EditorBlock {
  TextBlock({
    ...
    int quoteDepth = 0,
       ...
       assert(depth >= 0),
       assert(
         quoteDepth == 0 || containers.isEmpty,
         'quoteDepth 便捷参数与 containers 互斥',
       ),
       containers = List.unmodifiable(
         quoteDepth > 0
             // 便捷参数(测试用):每层独立发号 —— 注意这样每次构造的
             // 帧互不相等,跨块共享容器请显式传 containers
             ? List.generate(
                 quoteDepth,
                 (_) => QuoteFrame(groupId: nextFrameGroupId()),
               )
             : containers,
       );
  ...
  TextBlock asParagraph() =>
      TextBlock(id: id, content: content, containers: containers);
