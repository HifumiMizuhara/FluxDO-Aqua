...
class TopicTrackingStateNotifier extends Notifier<Map<int, TrackedTopicState>> {
    ...
    unawaited(
      preloaded
          ...
          .catchError((Object e, StackTrace st) {
            _loadingPreloadedStates = false;
            debugPrint(
              '[TopicTrackingState] 异步加载预加载追踪状态失败: $e',
            );
          }),
    );
  }
  ...
  void processChannelPayload(MessageBusMessage message) {
    ...
    debugPrint(
      '[TopicTrackingState] handlemessage: type=$messageType, channel=${message.channel}, data=$data',
    );
...
class MessageBusInitNotifier extends Notifier<void> {
  ...
  void build() {
    ...
    if (_allCallbacks.isNotEmpty) {
      debugPrint(
        '[MessageBusInit] 清理旧订阅: ${_allCallbacks.keys}',
      );
    ...
    if (currentUser == null) {
      debugPrint(
        '[MessageBusInit] 用户未登录，仅配置公开频道轮询域名',
      );
      return;
    }
    ...
    if (meta == null) {
      debugPrint('[MessageBusInit] topicTrackingStateMeta not load');
      return;
    }
    ...
    // 此处只负责话题追踪频道
    debugPrint(
      '[MessageBusInit] 订阅 ${meta.length} 个频道: ${meta.keys}',
    );
    for (final entry in meta.entries) {
      ...
      void onTopicTracking(MessageBusMessage message) {
        debugPrint(
          '[TopicTracking] receivedmessage: ${message.channel} #${message.messageId}',
        );
