...
class Topic {
  ...
  /// Stable Discourse group IDs for private-message recipients.
  final List<int> participantGroupIds;

  /// Discourse marks PMs addressed to a non-human user with this field.
  ...
  Topic({
    ...
    this.participantGroupIds = const [],
  ...
  factory Topic.fromJson(
    ...
    return Topic(
      ...
      hasParticipantGroups:
          json['has_participant_groups'] as bool? ??
          (json['participant_groups'] as List<dynamic>?)?.isNotEmpty ??
          false,
      participantGroupIds:
          (json['participant_groups'] as List<dynamic>? ?? const [])
              .map((group) {
                if (group is! Map) return null;
                return (group['id'] ?? group['group_id']) as int?;
              })
              .whereType<int>()
              .toList(),
      isSystemMessage: _isSystemPrivateMessage(json),
  ...
  /// Returns true for PMs generated by Discourse's system/moderation flows.
  ///
  /// The PM list serializer exposes `is_warning`, while the topic detail
  /// serializer exposes `pm_with_non_human_user`. Some compatible Discourse
  /// installations use the older explicit system flags instead, so all of
  /// these signals are accepted independently rather than with `??`.
  static bool _isSystemPrivateMessage(Map<String, dynamic> json) {
    if (json['pm_with_non_human_user'] == true ||
        json['is_system_message'] == true ||
        json['system_message'] == true ||
        json['is_warning'] == true) {
      return true;
    }

    final subtype = (json['subtype'] ?? json['topic_subtype'])
        ?.toString()
        .trim()
        .toLowerCase();
    if (subtype == 'notify_moderators' ||
        subtype == 'moderator_warning' ||
        subtype == 'system') {
      return true;
    }

    // Older Discourse versions do not serialize `subtype` in the PM list,
    // but the generated report PM excerpt still contains this stable phrase.
    // Keep this fallback limited to moderation-attention wording so ordinary
    // user conversations are not turned into system messages.
    final text = '${json['title'] ?? ''} ${json['excerpt'] ?? ''}'
        .trim()
        .toLowerCase();
    return text.contains('needs moderator attention') ||
        text.contains('requires moderator attention') ||
        text.contains('needs staff attention') ||
        text.contains('requires staff attention') ||
        text.contains('需要管理员注意') ||
        text.contains('需要管理人员注意') ||
        text.contains('需要管理員注意') ||
        text.contains('需要管理人員注意') ||
        text.contains('管理者の注意が必要');
  }
}
